...
@app.route("/team/")
def team_directory():
    """Serve the standalone team directory for direct browser navigation."""
...
@app.route("/api/team")
def list_team():
    """Return every teammate for the team directory."""
    conn = db.get_conn()
    rows = conn.execute(
        "SELECT user_id, name, avg_amount, tx_count FROM users ORDER BY name"
    ).fetchall()
    conn.close()
    return jsonify([dict(r) for r in rows])
